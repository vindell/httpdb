...
public class ArrayUtils {
	...
	/**
	 * Finds the first element of the array that is not equal to the given
	 * value.
	 */
	...
	public static boolean areEqualSets(int[] arra, int[] arrb) {
		return arra.length == arrb.length
				&& ArrayUtils.haveEqualSets(arra, arrb, arra.length);
	}
	 ...
	 *
	 * For full == false returns the result of haveEqualArrays(arra,arrb,count)
	 *
	...
	public static boolean areEqual(int[] arra, int[] arrb, int count,
		...
		if (ArrayUtils.haveEqualArrays(arra, arrb, count)) {
			if (full) {
	...
	/**
	 * Returns an int[] containing elements shared between the two arrays arra
	 * and arrb. The arrays contain sets (no value is repeated).
	 *
	 * Used to find the overlap between two arrays of column indexes. Ordering
	 * of the result arrays will be the same as in array a. The method assumes
	 * that each index is only listed once in the two input arrays.
	 * <p>
	 * e.g.
	 * </p>
	 * <code>
	 ...
	 * <p>
	 *
	 * Return the number of elements shared by two column index arrays. This
	 * method assumes that each of these arrays contains a set (each element
	 * index is listed only once in each index array). Otherwise the returned
	 * number will NOT represent the number of unique column indexes shared by
	 * both index array.
	 *
	 * @param arra
	 ...
	 *
	 * @return int; number of elements shared by <code>a</code> and
	 *         <code>b</code>
	 */
	...
	public static int[] union(int[] arra, int[] arrb) {
		...
		int newSize = arra.length + arrb.length
				- ArrayUtils.countCommonElements(arra, arrb);

		if (newSize > arra.length && newSize > arrb.length) {
			int[] arrn = (int[]) ArrayUtils.resizeArray(arrb, newSize);
			int pos = arrb.length;

			mainloop: for (int i = 0; i < arra.length; i++) {
				for (int j = 0; j < arrb.length; j++) {
					if (arra[i] == arrb[j]) {
						continue mainloop;
					}
	...
	public static int findNotIn(byte[] arra, int start, int limit,
		...
		mainloop: for (int k = start; k < limit; k++) {
	...
	public static int copyBytes(long sourceOffset, byte[] source,
			int sourceOff, int sourceLength, long destOffset, byte[] dest,
			int destLength) {
	...
	public static void copyMoveSegment(Object source, Object dest, int size,
			int index, int segmentSize, int destIndex) {

		boolean forward = index < destIndex;
		int sliceSize = forward ? index : destIndex;

		System.arraycopy(source, 0, dest, 0, sliceSize);

		sliceSize = forward ? size - destIndex - segmentSize : size - index
				- segmentSize;

		int sliceIndex = forward ? destIndex + segmentSize : index
				+ segmentSize;
	...
	public static Object duplicateArray(Object source) {
		...
		Object newarray = Array.newInstance(source.getClass()
	 ...
	 * copied over the element at colindex.
	 *
	 * No checks are performed on array sizes and an exception is thrown if they
	 * are not consistent with the other arguments.
	 */
	public static void copyAdjustArray(Object source, Object dest,
			Object addition, int colindex, int adjust) {
	 ...
	 * <p>
	 *
	 * colindex is the index at which an element is added or removed. Each
	 * element in the result array represents the new, adjusted index.
	 * <p>
	 *
	 * For each element of collarr that represents an index equal to colindex
	 * and adjust is -1, the result will not contain that element and will be
	 * shorter than collar by one element.
	 *
	 * @param colarr
	 *            the source array
	 * @param colindex
	 *            index at which to perform adjustment
	 * @param adjust
	 *            +1, 0 or -1
	...
	/**
	 * Copies some elements of row into newRow by using columnMap as the list of
	 * indexes into row.
	 * <p>
	 *
	 * columnMap and newRow are of equal length and are normally shorter than
	 * row.
	 * <p>
	 *
	 * @param row
	...
	/*
	 * public static void copyColumnValues(int[] row, int[] colindex, int[]
	 * colobject) {
	 * 
	 * for (int i = 0; i < colindex.length; i++) { colobject[i] =
	 * row[colindex[i]]; } }
	 * 
	 ...
	 * row[colindex[i]]; } }
	 * 
	 * public static void copyColumnValues(byte[] row, int[] colindex, byte[]
	 * colobject) {
	 * 
	 * for (int i = 0; i < colindex.length; i++) { colobject[i] =
	 * row[colindex[i]]; } }
	...
	public static boolean isInSortedArray(char ch, char[] array) {
		...
		if (array.length == 0 || ch < array[0] || ch > array[array.length - 1]) {
			return false;
		}
	...
	public static boolean containsAll(Object[] arra, Object[] arrb) {
		...
		mainLoop: for (int i = 0; i < arrb.length; i++) {
	...
	/**
	 * returns the largest value that is a power of 2 and larger or equal to
	 * value
	 */
