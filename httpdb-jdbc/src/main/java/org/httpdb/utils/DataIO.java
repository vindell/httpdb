    ...
    public static Object read(InputStream in, String password) throws Exception {
    	...
    	if (din.readBoolean()) {
    		...
    		if (n == 0) {
                ...
                List<ColumnType> fields = rowSet.getFields();
                for (int i = 0; i < n; i++) {
                	fields.add(new ColumnType(readString(din), readString(din)));
                }
                List<Map<String,Object>> rows = rowSet.getRows();
                ...
                for (int i = 0; i < rowSize; i++) {
            		...
            		rows.add(map);
                    for (int j = 0; j < n; j++) {
                    	map.put( fields.get(j).getName(), readString(din));
                    }
                }
                return rowSet;
    		} else {
